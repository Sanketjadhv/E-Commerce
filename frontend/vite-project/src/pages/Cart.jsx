...
function Cart() {
  ...
  return (
    <>
    <div className='w-[100%] min-h-[100vh] p-[20px] overflow-hidden bg-gradient-to-l from-[#141414] to-[#0c2025]'>
            ...
            cartData.map((item,index)=>{
              ...
              <div key={index} className='w-[100%] h-[10%] border-t border-b'>
                <div className='w-[100%] h-[80%] flex items-start gap-6 bg-[#51808048] py-[10px] px-[20px] rounded-2xl relative'>
                   <img src={productData.image1} alt='' className='w-[100px] rounded-md' />
                  <div className='flex items-start justify-center flex-col gap-[10px]'>
                    <p className='text-[20px] text-[#aaf4e7]'> {currency} {productData.price} </p>
                    <p className=' w-[40px] h-[40px] text-[16px] text-[#aaf4e7] bg-[#518080b4] rounded-md mt-[5px] flex items-center justify-center border-[1px] border-[#9ff9f9]'>{item.size}</p>
                  </div>
                   <input type='number' min={1} defaultValue={item.quantity} className='md:max-w-20 max-w-10 md:px-2 md:py-2 py-[5px] px-[10px] text-[white] text-[18px] font-semibold bg-[#518080b4] absolute md:top-[40%] top-[46%] left-[75%] md:left-[50%] border-[1px] border-[#9ff9f9] rounded-md' onChange={(e)=>e.target.value === ' ' || e.target.value === '0' ? null : updateQuantity(item._id, item.size, Number(e.target.value))}/>
                    <IoTrashBin className='text-[#9ff9f9] w-[25px] h-[25px] absolute top-[50%] md;top-[40%] md:right-[5%] right-1' onClick={()=>updateQuantity(item._id, item.size, 0)} />
                  </div>
      ...
      <div className='flex justify-start items-end my-20'>
        <div className='w-full sm:w-[450px]'>
          ...
          }}>{loading ? <Loading/> : "Proceed To Placed"}</button>
        </div>
      </div>
